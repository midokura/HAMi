...
func TestE2EFlow_FitToEnvVars(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Step 4: PatchAnnotations writes the shared container encoding
			// and, under Option A, the dedicated amd.com/cu-mask annotation.
			...
			amdDev.PatchAnnotations(pod, &annos, device.PodDevices{
				amd.AMDDevice: device.PodSingleDevice{cds},
			})
			for k, v := range annos {
				pod.Annotations[k] = v
			}
			...
			for i, dev := range devreq {
				...
				// Option A: the mask travels in the dedicated amd.com/cu-mask
				// annotation, not in the shared container encoding, so the
				// plugin reads it back from the pod rather than from CustomInfo.
				if mask := amd.LookupCUMask(pod.Annotations, dev.UUID); mask != "" {
					envs["ROC_GLOBAL_CU_MASK"] = mask
				}
			}
...
func TestE2EFlow_MultiTenantCUExclusion(t *testing.T) {
	...
	// Apply tenant A's allocation to device usage. Under Option A this takes
	// two steps: AddResourceUsage keeps the count bookkeeping, while the CU
	// occupancy is carried by the amd.com/cu-mask annotation and rebuilt by
	// Fit (rebuildCUBitmapFromPods) from the pods already on the device.
	cdsA := resultA[amd.AMDDevice]
	for _, cd := range cdsA {
		amdDev.AddResourceUsage(podA, devices[0], &cd)
	}
	annosA := map[string]string{}
	amdDev.PatchAnnotations(podA, &annosA, device.PodDevices{
		amd.AMDDevice: device.PodSingleDevice{cdsA},
	})
	for k, v := range annosA {
		podA.Annotations[k] = v
	}
	devices[0].PodInfos = append(devices[0].PodInfos, &device.PodInfo{Pod: podA})
...
func TestE2EFlow_EncodedAnnotationBackwardCompat(t *testing.T) {
	...
	pod := corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			...
			Namespace: "default",
			Annotations: map[string]string{
				device.InRequestDevices[amd.AMDDevice]: oldAnnotation + ";",
			},
		},
		Spec: corev1.PodSpec{
