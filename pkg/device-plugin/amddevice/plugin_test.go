...
func init() {
	...
	if _, ok := device.InRequestDevices[amd.AMDDevice]; !ok {
		device.InRequestDevices[amd.AMDDevice] = "hami.io/amd-devices-to-allocate"
	}
}

// annotatePod runs the scheduler's PatchAnnotations over devs and copies the
// result onto pod, so the plugin sees exactly what the scheduler persists:
// the shared container encoding plus, under Option A, amd.com/cu-mask.
func annotatePod(pod *corev1.Pod, devs device.ContainerDevices) {
	...
	amdDev.PatchAnnotations(pod, &annos, device.PodDevices{
		amd.AMDDevice: device.PodSingleDevice{devs},
	})
...
func TestGetNextDeviceRequest(t *testing.T) {
	...
	// Option A keeps the mask out of the shared container encoding, so the
	// decoded device carries no CU CustomInfo; the plugin reads the mask from
	// the amd.com/cu-mask annotation instead.
	if dev.CustomInfo[amd.CUMaskKey] != nil {
		t.Errorf("CU mask leaked into the shared encoding: %v", dev.CustomInfo[amd.CUMaskKey])
	}
	if got := amd.LookupCUMask(pod.Annotations, dev.UUID); got != "0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF" {
		t.Errorf("cu_mask mismatch: got %q", got)
	}
}
...
func TestGetNextDeviceRequest_MultiDevice(t *testing.T) {
	...
	// Check first device (mask via the annotation, per Option A)
	if got := amd.LookupCUMask(pod.Annotations, devreq[0].UUID); got != "0xFFFFFFFFFFFFFFFFFFF" {
		t.Errorf("device 0 cu_mask mismatch: got %q", got)
	}
	if devreq[0].Usedmem != 48000 {
		t.Errorf("device 0 Usedmem mismatch: got %d", devreq[0].Usedmem)
	}

	// Check second device (mask via the annotation, per Option A)
	if got := amd.LookupCUMask(pod.Annotations, devreq[1].UUID); got != "0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF" {
		t.Errorf("device 1 cu_mask mismatch: got %q", got)
	}
...
func TestGetNextDeviceRequest_NoCUMask(t *testing.T) {
	...
	pod := corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			...
			Namespace: "default",
			Annotations: map[string]string{
				device.InRequestDevices[amd.AMDDevice]: podSingle,
			},
		},
		Spec: corev1.PodSpec{
