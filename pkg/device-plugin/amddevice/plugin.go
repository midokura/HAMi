...
func (p *AMDDevicePlugin) Allocate(ctx context.Context, reqs *kubeletdevicepluginv1beta1.AllocateRequest) (*kubeletdevicepluginv1beta1.AllocateResponse, error) {
	...
	for range reqs.ContainerRequests {
		...
		for i, dev := range devreq {
			...
			// CU mask from the amd.com/cu-mask annotation (Option A: the
			// scheduler writes it in a dedicated annotation rather than the
			// shared container encoding).
			if mask := amd.LookupCUMask(current.Annotations, dev.UUID); mask != "" {
				response.Envs["ROC_GLOBAL_CU_MASK"] = mask
			}
		}
